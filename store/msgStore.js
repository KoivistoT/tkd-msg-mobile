...
const slice = createSlice({
  ...
  reducers: {
    ...
    msgTasksResived: (msgStore, action) => {
        ...
        if (taskType === taskTypes.newMessage) {
          ...
          if (type === "image" && imageURLs.length > 0) {
            imageURLs.forEach((currentUrl) => {
              newState.images[roomId].unshift(currentUrl);
            });
          }
      ...
      action.payload.forEach((task) => {
        ...
        if (taskType === taskTypes.messageDeleted) {
          ...
          newState2.allMessages[roomId].messages[messageId].deleted = true;
        }
      ...
      try {
        ...
        if (type === "image" && imageURLs.length > 0) {
          imageURLs.forEach((imageURL) => {
            msgStore.images[currentRoomId].unshift(imageURL);
          });
        }
    ...
    messageDeleted: (msgStore, action) => {
      ...
      msgStore.allMessages[roomId].messages[messageId].deleted = true;
    },
