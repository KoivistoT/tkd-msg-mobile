...
const slice = createSlice({
  ...
  reducers: {
    ...
    usersErrorMessageCleared: (users) => {
    ...
    userLastPresentResived: (users, action) => {
      const { userId, lastPresent } = action.payload;
      users.allUsers[userId].lastPresent = lastPresent;
    },
  },
});
...
export const selectLastPresentByUserId = (userId) =>
  createSelector(
    (state) => state.entities.users,
    (users) => users.allUsers[userId]?.lastPresent
  );
